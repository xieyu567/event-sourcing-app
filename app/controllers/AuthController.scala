...
class AuthController(
  ...
  def doLogin(): Action[AnyContent] = Action.async(parse.anyContent) { implicit request =>
    userLoginDataForm
      .bindFromRequest()
      .fold(
  ...
  def registerUser(): Action[AnyContent] = Action.async(parse.anyContent) { implicit request =>
    userRegistrationDataForm
      .bindFromRequest()
      .fold(
        ...
        userData => {
          ...
          if (!passwordsMatch) {
            Future.successful(Ok(views.html.security.signUp(Some("Passwords don't match"))))
          } else {
            authService
              .register(userData.username, userData.fullName, userData.password1)
              .map { cookie =>
  ...
  private val userLoginDataForm = Form {
    mapping("username" -> email, "password" -> nonEmptyText)(UserLoginData.apply)(
      UserLoginData.unapply)
  }
  ...
  private val userRegistrationDataForm = Form {
    mapping(
      ...
      "password2" -> nonEmptyText)(UserRegistrationData.apply)(UserRegistrationData.unapply)
  }
