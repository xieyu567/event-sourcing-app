...
class UserDao {
  ...
  def findById(userId: UUID): Future[Option[User]] = Future {
    NamedDB(Symbol("auth")).readOnly { implicit session =>
      sql"select * from users where user_id = $userId".map(User.fromRS).headOption().apply()
    }
  }
  ...
  def insertUser(userCode: String, fullName: String, password: String): Future[User] = Future {
